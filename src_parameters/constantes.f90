...
!_ ================================================================================================================================
  ...
  SUBROUTINE veget_config
    ...
    !Config Key   = READ_ONFERT
    !Config Desc  = Use ONfert event fertilization from forcing file
    !Config Def   = n
    !Config If    = OK_STOMATE
    !Config Help  = If true, ONfert from the forcing file replaces the default fertilizer input pathway.
    !Config         If false, ONfert is ignored and the standard ORCHIDEE nitrogen fertilization pathway is used.
    !Config Units = [FLAG]
    CALL getin_p('READ_ONFERT', read_ONfert)

    WRITE(numout,*) 'MOJ DEBUG READ_ONFERT after getin_p = ', read_ONfert
...
!_ ================================================================================================================================
  ...
  SUBROUTINE config_sechiba_parameters
    ...
    !Config If    = OK_SECHIBA 
    !Config Def   = 0.4
    !Config Help  = This is the fraction of the soil assigned to the
    !Config         silt classification if the fraction cannot be determined
    !Config         by any other means.
    !Config Units = [-]   
...
!_ ================================================================================================================================
  ...
  SUBROUTINE config_stomate_parameters
    ...
    !-                                                                                                                                                               
    !Config Key   = CTE_BACT                                                                              
    !Config Desc  = Denitrification activiy of bacteries                                                                                                 
    !Config If    = OK_STOMATE                                                                                                                                       
    !Config Def   = 0.00005                                                                                                                
    !Config Help  =                                                                                                                                                  
    !Config Units =                                                                                                                                        
    ...
    CALL getin_p('VMAX_UPTAKE',vmax_uptake)
    !-
    !Config Key   = K_N_MIN
   ...
   !Config Key   = TIMING_FERT 
    !Config Desc  = Number of days after begin_leaves during which N is applied to soil 
    !Config If    = OK_NCYCLE 
    !Config Def   = 183 (6 months)  
    !Config Help  = 
    !Config Units = [-] 
    CALL getin_p('TIMING_FERT',timing_fert) 
    ...
    !
    !Config Key   = LD_ALLOC
    !Config Desc  = A flag to turn of debug statement
    !Config If    = 
    !Config Def   = 6
    !Config Help  = 
    !Config Units = [-]   
