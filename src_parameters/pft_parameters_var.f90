...
!! SVN          :
!! $HeadURL: $
!! $Date: 2026-02-25 15:22:16 +0100 (Wed, 25 Feb 2026) $
!! $Revision: 9399 $
!! \n
!_ ================================================================================================================================
...
!$OMP THREADPRIVATE(canopy_cover)
  ...
  INTEGER(i_std), ALLOCATABLE, SAVE, DIMENSION(:) :: nmaxtrees      !! Intial number of seedlings per hectare. Used 
...
!$OMP THREADPRIVATE(ok_LAIdev)
  LOGICAL, ALLOCATABLE, SAVE, DIMENSION(:) :: laiTest     !! flag for using the lai(:) / added by mojtaba   
!$OMP THREADPRIVATE(laiTest)

LOGICAL, ALLOCATABLE, SAVE, DIMENSION(:) :: ok_inncalc     !! flag for using the STICS-LAIdev module    
!$OMP THREADPRIVATE(ok_inncalc) 

LOGICAL, ALLOCATABLE, SAVE, DIMENSION(:) :: laiTestg     !! flag for using the lai(:) / added by mojtaba
!$OMP THREADPRIVATE(laiTest) 
...
!!!!!for STICS Nitrogen stress scheme
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:) :: adil
!$OMP THREADPRIVATE(adil)

  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:) :: bdil
!$OMP THREADPRIVATE(bdil)
