...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_parallel/xios_orchidee.f90 $
!! $Date: 2023-03-06 09:39:23 +0100 (Mon, 06 Mar 2023) $
!! $Revision: 7905 $
!! \n
!_ ================================================================================================================================
...
MODULE xios_orchidee
  ...
  USE constantes_var
  USE constantes_soil_var, ONLY : nstm, nscm, check_cwrr, ok_freeze_cwrr, ok_moss
  ...
  PUBLIC :: xios_orchidee_init, xios_orchidee_change_context, &
            ...
            xios_orchidee_set_file_attr, xios_orchidee_set_field_attr, xios_orchidee_set_fieldgroup_attr, xios_orchidee_setvar
   ...
   IF (printlev>=1) WRITE(numout,*)'In xios_orchidee_init, xios_interpolation=', xios_interpolation
    ...
    IF (xios_orchidee_ok .AND. is_omp_root) THEN
       ...
       ELSE IF (grid_type==regular_xy ) THEN
          ! Case not yet fully implemented
          CALL ipslerr_p(3,'xios_orchidee_init', 'Implemention for grid_type=regular_xy is not finalized',&
               'Initialization of the domain must be looked over in the code', '')

! Following was done in previous version for case grid_type=regular_xy
!         ! Local MPI domain
...
!          CALL xios_set_domain_attr("domain_landpoints",lonvalue_2d=lon_mpi,latvalue_2d=lat_mpi)
       ...
       CALL xios_set_axis_attr("ncsm", n_glo=nscm,VALUE=(/(REAL(i,r_std),i=1,nscm)/))
