...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_sechiba/hydrol.f90 $
!! $Date: 2023-04-06 10:58:06 +0200 (Thu, 06 Apr 2023) $
!! $Revision: 7944 $
!! \n
!_ ===============================================================================================\n
MODULE hydrol
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrol_initialize ( kjit,           kjpindex,  index,         rest_id,          &
                                 ...
                                 snowdz,         snowheat,  &
                                 mc_layh,        mcl_layh,  soilmoist_out, mc_layh_s,  mcl_layh_s,  soilmoist_out_s)
...
!_ ================================================================================================================================
    ...
    CALL hydrol_var_init (kjpindex, veget, veget_max, soiltile, njsc, altmax, &
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrol_main (kjit, kjpindex, &
    ...
    REAL(r_std),DIMENSION (kjpindex,nvm),INTENT(in)    :: altmax           !! Maximum active layer thickness (m).
    ...
    REAL(r_std),DIMENSION (kjpindex,nslm,nstm), INTENT(out)  :: mcl_layh_s     !! Volumetric moisture content for each layer in hydrol(liquid) [m3/m3]/
    REAL(r_std),DIMENSION (kjpindex,nslm,nstm), INTENT(out)  :: soilmoist_out_s  !! Total soil moisture content for each layer in hydrol(liquid + ice) [mm]
    REAL(r_std),DIMENSION (nscm), INTENT(out)                :: mcs_hydrol        !! Saturated volumetric water content output to be used in stomate_soilcarbon
    REAL(r_std),DIMENSION (nscm), INTENT(out)                :: mcfc_hydrol       !! Volumetric water content at field capacity output to be used in stomate_soilcarbon
    ...
    REAL(r_std),DIMENSION (kjpindex, nsnow)            :: snowdz_diag      !! Depth of snow layer on all layers containing default values, only for diagnostics [m]
...
! Modif temp vuichard
    ...
    CALL hydrol_soil(kjpindex, veget_max, soiltile, njsc, reinf_slope,  &
         ...
         mc_layh, mcl_layh, mc_layh_s, mcl_layh_s, altmax)
...
!_ hydrol_var_init
  ...
  SUBROUTINE hydrol_var_init (kjpindex, veget, veget_max, soiltile, njsc, altmax, &
       mx_eau_var, shumdiag_perma, &
       drysoil_frac, qsintveg, mc_layh, mcl_layh, mc_layh_s, mcl_layh_s) 
...
!_ hydrol_soil
  ...
  SUBROUTINE hydrol_soil (kjpindex, veget_max, soiltile, njsc, reinf_slope, &
       ...
       & k_litt, litterhumdiag, humrel,vegstress, drysoil_frac, &
       & stempdiag,snow, &
       & snowdz, tot_bare_soil, u, v, tq_cdrag, mc_layh, mcl_layh, mc_layh_s, mcl_layh_s, altmax)
    ...
    REAL(r_std)                                    :: nroot_tmp_nodynroot        !! Temporary variable to calculate nroot if ok_dynroot=false
...
!_ ================================================================================================================================
    ...
    IF (ok_freeze_cwrr) THEN
       ...
       DO jst=1,nstm
          CALL hydrol_soil_froz(kjpindex,jst,njsc,stempdiag)
       ENDDO
    ...
    DO jst = 1,nstm
       ...
       CALL hydrol_soil_infilt(kjpindex, jst, njsc, flux_infilt, stempdiag, &
       ...
       ELSE
          ...
          ! This is the case ok_dynroot=FALSE
          ! Compute nroot at each time step even if ok_dynroot=false to avoid restartability issues
          DO ji=1, kjpindex
             !-
             !! The three following equations concerning nroot computation are
             !derived from the integrals 
             !! of equations C9 to C11 of De Rosnay's (1999) PhD thesis (page158).
             ...
             DO jv = 1,nvm
                ...
                !! If ok_soil_carbon_discretization: nroot is set zero below ALT,
                !! and then re-normalized to 1 along the depth
  
                IF ( ok_soil_carbon_discretization ) THEN
                   nroot_tmp_nodynroot=zero
                   DO jsl = 1, nslm
                      IF (znh(jsl) .LT. altmax(ji,jv)) THEN
                         nroot_tmp_nodynroot =nroot_tmp_nodynroot+nroot(ji,jv,jsl)
                      ELSEIF (altmax(ji,jv) .GT. zero) THEN
                         nroot(ji,jv,jsl)=zero
                      ENDIF
                   ENDDO
                   IF (nroot_tmp_nodynroot .GT. zero) THEN
                      nroot(ji,jv,:)=nroot(ji,jv,:)/nroot_tmp_nodynroot
                   ENDIF
                ENDIF   
             ENDDO
          ENDDO
    ...
    DO jst = 1,nstm
       ...
       IF ( ok_freeze_cwrr ) CALL hydrol_soil_froz(kjpindex,jst,njsc,stempdiag)
        DO jsl = 1, nslm
          DO ji =1, kjpindex
             mcl(ji,jsl,jst)= MIN( mc(ji,jsl,jst), mcr(njsc(ji)) + &
                  (un-profil_froz_hydro_ns(ji,jsl,jst))*(mc(ji,jsl,jst)-mcr(njsc(ji))) )
...
!_ hydrol_soil_infilt
  ...
  SUBROUTINE hydrol_soil_infilt(kjpindex, ins, njsc, flux_infilt, stempdiag,&
...
!_ hydrol_soil_froz
  ...
  SUBROUTINE hydrol_soil_froz(kjpindex,ins,njsc,stempdiag)
             ...
             ELSE IF (ok_thermodynamical_freezing) THEN
                ...
                ELSE IF ( (stempdiag(ji,jsl) .GE. (fr_center-fr_dT/2.)) .AND. &
                   ...
                   x=MIN(((mcs(njsc(ji))-mcr(njsc(ji))) &
                        *((2.2*1000.*avan(njsc(ji))*(fr_center+fr_dT/2.-stempdiag(ji, jsl)) &
