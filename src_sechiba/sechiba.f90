...
!!   
!! SVN     :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_sechiba/sechiba.f90 $ 
!! $Date: 2025-10-06 10:03:47 +0200 (Mon, 06 Oct 2025) $
!! $Revision: 9120 $
!! \n
!_ ================================================================================================================================
...
!! 0.1 Input variables
    ...
    INTEGER(i_std),INTENT(in)                                :: hist_id_stom_IPCC !! STOMATE's IPCC _history_ file file 
                                                                                  !! identifier (unitless)
    REAL(r_std),DIMENSION (:,:), INTENT (in)          :: lalo              !! Geographic coordinates (latitude,longitude)
                                                                                  !! for grid cells (degrees)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: contfrac          !! Fraction of continent in the grid 
                                                                                  !! (unitless, 0-1)
    INTEGER(i_std),DIMENSION (:), INTENT (in)         :: index             !! Indices of the pixels on the map. 
                                                                                  !! Sechiba uses a reduced grid excluding oceans
                                                                                  !! ::index contains the indices of the 
                                                                                  !! terrestrial pixels only! (unitless)
    INTEGER(i_std), DIMENSION (:,:), INTENT(in):: neighbours        !! Neighboring grid points if land!(unitless)
    REAL(r_std), DIMENSION (:,:), INTENT(in)          :: resolution        !! Size in x and y of the grid (m)
    
    REAL(r_std),DIMENSION (:), INTENT (in)            :: u                 !! Lowest level wind speed in direction u 
                                                                                  !! @tex $(m.s^{-1})$ @endtex 
    REAL(r_std),DIMENSION (:), INTENT (in)            :: v                 !! Lowest level wind speed in direction v 
                                                                                  !! @tex $(m.s^{-1})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: zlev              !! Height of first layer (m)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: qair              !! Lowest level specific humidity 
                                                                                  !! @tex $(kg kg^{-1})$ @endtex
    ...
    REAL(r_std),DIMENSION (:), INTENT (in)            :: precip_snow       !! Snow precipitation 
                                                                                  !! @tex $(kg m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: lwdown            !! Down-welling long-wave flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: swnet             !! Net surface short-wave flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: swdown            !! Down-welling surface short-wave flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: temp_air          !! Air temperature (K)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: petAcoef          !! Coefficients A for T from the Planetary 
                                                                                  !! Boundary Layer
    REAL(r_std),DIMENSION (:), INTENT (in)            :: peqAcoef          !! Coefficients A for q from the Planetary 
                                                                                  !! Boundary Layer
    REAL(r_std),DIMENSION (:), INTENT (in)            :: petBcoef          !! Coefficients B for T from the Planetary 
                                                                                  !! Boundary Layer
    REAL(r_std),DIMENSION (:), INTENT (in)            :: peqBcoef          !! Coefficients B for q from the Planetary 
                                                                                  !! Boundary Layer
    REAL(r_std),DIMENSION (:), INTENT (in)            :: pb                !! Surface pressure (hPa)


!! 0.2 Output variables
    REAL(r_std),DIMENSION (:), INTENT (out)           :: coastalflow       !! Outflow on coastal points by small basins.
                                                                                  !! This is the water which flows in a disperse 
                                                                                  !! way into the ocean
                                                                                  !! @tex $(kg dt_routing^{-1})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: riverflow         !! Outflow of the major rivers.
                                                                                  !! The flux will be located on the continental 
                                                                                  !! grid but this should be a coastal point  
                                                                                  !! @tex $(kg dt_routing^{-1})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: tsol_rad          !! Radiative surface temperature 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: vevapp            !! Total of evaporation 
                                                                                  !! @tex $(kg m^{-2} days^{-1})$ @endtex
    
    REAL(r_std),DIMENSION (:), INTENT (out)           :: qsurf_out         !! Surface specific humidity 
                                                                                  !! @tex $(kg kg^{-1})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: z0m_out           !! Surface roughness momentum (output diagnostic, m)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: z0h_out           !! Surface roughness heat (output diagnostic, m)
    REAL(r_std),DIMENSION (:,:), INTENT (out)         :: albedo            !! Surface albedo for visible and near-infrared
                                                                                  !! (unitless, 0-1)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: fluxsens          !! Sensible heat flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: fluxlat           !! Latent heat flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: emis_out          !! Emissivity (unitless)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: temp_sol_new      !! New ground temperature (K)

!! 0.3 Modified
    REAL(r_std),DIMENSION (:), INTENT (inout)         :: tq_cdrag          !! Surface drag coefficient (-)
...
!_ ================================================================================================================================
    ...
    CALL hydrol_initialize ( kjit,           kjpindex,  index,         rest_id,          &
         ...
         snowdz,         snowheat,  &
         mc_layh,    mcl_layh,  soilmoist, mc_layh_s,  mcl_layh_s, soilmoist_s)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE sechiba_main (kjit, kjpij, kjpindex, index, &
       ...
       & zlev, u, v, qair, temp_air, epot_air, ccanopy, ONfert, &
...
!! 0.1 Input variables
    ...
    REAL(r_std),DIMENSION (:), INTENT (in)            :: lwdown            !! Down-welling long-wave flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: coszang           !! Cosine of the solar zenith angle (unitless)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: swnet             !! Net surface short-wave flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: swdown            !! Down-welling surface short-wave flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (in)            :: temp_air          !! Air temperature (K)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: epot_air          !! Air potential energy (??J)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: ccanopy           !! CO2 concentration in the canopy (ppm)
    REAL(r_std),DIMENSION (:), INTENT (in)            :: ONfert            !! crop fert for stics
    REAL(r_std),DIMENSION (:), INTENT (in)            :: petAcoef          !! Coefficients A for T from the Planetary 
                                                                                  !! Boundary Layer
    REAL(r_std),DIMENSION (:), INTENT (in)            :: peqAcoef          !! Coefficients A for q from the Planetary 
                                                                                  !! Boundary Layer
...
!! 0.2 Output variables
    ...
    REAL(r_std),DIMENSION (:), INTENT (out)           :: fluxlat           !! Latent heat flux 
                                                                                  !! @tex $(W m^{-2})$ @endtex
    REAL(r_std),DIMENSION (:), INTENT (out)           :: emis_out          !! Emissivity (unitless)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: netco2flux        !! Sum CO2 flux over PFTs 
                                                                                  !! (gC/m2/dt_sechiba)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: fco2_lu           !! Land Cover Change CO2 flux (gC/m2/one_day)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: fco2_wh           !! Wood harvest CO2 flux (gC/m2/one_day)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: fco2_ha           !! Crop harvest CO2 flux (gC/m2/one_day)
    REAL(r_std),DIMENSION (:,:), INTENT (out)       :: veget_out         !! Fraction of vegetation type (unitless, 0-1)
    REAL(r_std),DIMENSION (:,:), INTENT (out)       :: lai_out           !! Leaf area index (m^2 m^{-2}) 
    REAL(r_std),DIMENSION (:,:), INTENT (out)       :: height_out        !! Vegetation Height (m)
    REAL(r_std),DIMENSION (:), INTENT (out)           :: temp_sol_new      !! New ground temperature (K)
...
!! 0.4 local variables
    ...
    REAL(r_std), DIMENSION(kjpindex,nslm,nvm)                :: mcl_layh_pft      !! As mcl_layh per pft
    REAL(r_std), DIMENSION(kjpindex,nslm,nvm)                :: soilmoist_pft     !! As soilmoist per pft
    REAL(r_std), DIMENSION(nscm)                             :: mcs_hydrol        !! Saturated volumetric water content output to be used in stomate_soilcarbon
    REAL(r_std), DIMENSION(nscm)                             :: mcfc_hydrol       !! Volumetric water content at field capacity output to be used in stomate_soilcarbon
...
!_ ================================================================================================================================
    ...
    CALL hydrol_main (kjit, kjpindex, &
         ...
         & mc_layh, mcl_layh, tmc_pft, drainage_pft, runoff_pft, swc_pft, soilmoist, mc_layh_s, mcl_layh_s, soilmoist_s, &
         & mcs_hydrol, mcfc_hydrol, altmax)
    ...
    CALL slowproc_main (kjit, kjpij, kjpindex, njsc, &
         ...
         vegstress, shumdiag, litterhumdiag, precip_rain, precip_snow, pb,gpp, &
    ...
    netco2flux(:) = zero
    DO jv = 2,nvm
      netco2flux(:) = netco2flux(:) + co2_flux(:,jv)
    ENDDO
    ...
    CALL xios_orchidee_send_field("co2_flux",co2_flux/1.e3/one_day)
    ...
    CALL xios_orchidee_send_field("gpplut",gpplut/dt_sechiba)
    ...
    IF ( .NOT. almaoutput ) THEN
       ...
       IF ( ok_stomate ) THEN
          CALL histwrite_p(hist_id, 'co2_flux', kjit, co2_flux/1.e3*dt_sechiba/one_day, kjpindex*nvm, indexveg)    
       ENDIF
    ...
    IF ( hist2_id > 0 ) THEN
       IF ( .NOT. almaoutput ) THEN
          ...
          IF ( ok_stomate ) THEN 
             CALL histwrite_p(hist2_id, 'co2_flux', kjit, co2_flux/1.e3*dt_sechiba/one_day, kjpindex*nvm, indexveg)    
          ENDIF
       ELSE
          ! Write history file in ALMA format
          CALL histwrite_p(hist2_id, 'vegetfrac', kjit, veget, kjpindex*nvm, indexveg)
