...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_sechiba/explicitsnow.f90 $
!! $Date: 2020-07-07 09:47:17 +0200 (Tue, 07 Jul 2020) $
!! $Revision: 6830 $
!! \n
!_ ================================================================================================================================
...
!    CALL explicitsnow_levels(kjpindex,snow_depth_tmp, snowdz)
    DO ji=1,kjpindex
                IF (snowdz(ji,1) .NE. 0 .AND. snowdz(ji,2) .EQ. 0) THEN
                !Condensation on first layer, see Ticket #642.
                        snow_depth_tmp(ji) = SUM(snowdz(ji,:))
                        snowdz(ji,:)=snow_depth_tmp(ji)/3
                        snowrho(ji,2:3)=snowrho(ji,1)
                ENDIF
    ENDDO   
...
!================================================================================================================================
   ...
   SUBROUTINE explicitsnow_transf(kjpindex,snowdz_old,snowdz,snowrho,snowheat,snowgrain)
     ...
     INTEGER(i_std),INTENT(in)                                           :: kjpindex         !! Domain size
     REAL(r_std),DIMENSION(kjpindex,nsnow),INTENT(in)                    :: snowdz_old       !! Snow depth at the previous time step  [m]
