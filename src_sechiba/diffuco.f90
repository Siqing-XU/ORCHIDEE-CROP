...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_sechiba/diffuco.f90 $
!! $Date: 2025-09-26 09:26:24 +0200 (Fri, 26 Sep 2025) $
!! $Revision: 9109 $
!! \n
!_ ================================================================================================================================
...
SUBROUTINE diffuco_trans_co2 (kjpindex, swdown, pb, qsurf, qair, temp_air, temp_growth, rau, u, v, q_cdrag, humrel, &
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: v                !! Lowest level wind speed 
                                                                                 !! @tex ($m s^{-1}$) @endtex
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: q_cdrag          !! Surface drag coefficient  (-)
    REAL(r_std),DIMENSION (kjpindex,nvm,npco2), INTENT (in)  :: assim_param      !! referenced vcmax, nue, leaf N
                                                                                 !! for photosynthesis 
...
INTEGER(i_std),DIMENSION (kjpindex), INTENT (in)   :: index            !! Indeces of the points on the map (-)
    ...
    !! 0.4 Local variables
    !
    REAL(r_std),DIMENSION (kjpindex)      :: vcmax                               !! maximum rate of carboxylation. Notice that,
    ...
    REAL(r_std),DIMENSION (kjpindex,nvm)  :: ntot                                !! Total canopy (eg leaf) N (gN m-2[ground])  
...
! @}
    ...
    DO jv = 2,nvm
      ...
      vj(:) = ( arJV(jv) + brJV(jv) *  MAX(11., MIN(temp_growth(:),35.)) ) * vcmax(:) * T_Jmax(:)
      ...
      DO jl = 1, nlai
         ...
         Rd(:,jl) = vcmax(:) * N_profile * 0.01 * T_Rd(:) * MAX(1-stress_vcmax(jv), water_lim(:))
