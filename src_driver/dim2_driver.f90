...
!!   
!! SVN     :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_driver/dim2_driver.f90 $ 
...
!-   - Basic flags for SSIPSL
  ...
  !Config Desc  = Read ONfert forcings
  !Config Def   = READ_ONFERT
  !Config If    =
  !Config Help  = If READ_ONFERT=False use the scalar in the run.def
  !Config Units = [FLAG]
  read_ONfert = .FALSE.
...
!-
 ...
 &  pb(iim,jjm), lwdown(iim,jjm), ONfert(iim,jjm), &
...
!       first level conditions
 ...
 &         for_qair, for_tair, for_eair, for_ccanopy,ONfert, &
