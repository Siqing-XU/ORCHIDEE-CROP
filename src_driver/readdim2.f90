...
!!   
!! SVN     :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/perso/mojtaba.houballah/ORCHIDEE_3/ORCHIDEE/src_driver/readdim2.f90 $ 
...
SUBROUTINE forcing_read &
  ...
  IMPLICIT NONE
   ...
   LOGICAL, INTENT(IN) :: read_ONfert         !! Flag to ON fertilization
   ...
   REAL,DIMENSION(iim,jjm), INTENT(OUT) :: pb          !! Surface pressure (Pa)i
   REAL,DIMENSION(iim,jjm), INTENT(OUT) :: ONfert          !! ONFERT
...
!- read_ONfert: flag that decides if we want to read ONfert data
...
!-
   ...
   LOGICAL, INTENT(IN) :: lwdown_cons, swdown_cons, read_ONfert
   REAL    :: date0, dt_force
   INTEGER :: iim, jjm, ttm
...
!-
  ...
  &  swdown, coszang, rainf, snowf, tair, u, v, qair, pb, ONfert,lwdown, &
  &  fcontfrac
...
!-
  ...
  &  pb_nm1, lwdown_nm1,ONfert_nm1, & 
  &  zlev_n, zlevuv_n, swdown_n, rainf_n, snowf_n, tair_n, u_n, v_n, qair_n, &
  &  pb_n, lwdown_n, mean_coszang, ONfert_n
...
!-     
     CALL forcing_just_read (iim, jjm, zlev, zlevuv, ttm, 1, 1, &
          ...
          &  u, v, qair, pb, lwdown, &
          &  SWnet, Eair, petAcoef, peqAcoef, petBcoef, peqBcoef, cdrag, ccanopy, &
          &  force_id, wind_N_exists,read_ONfert,ONfert)
!----
...
!---
            ...
            IF (last_read == 0) THEN
               ...
               IF (itau_read > 1) THEN
                  ...
                  CALL forcing_just_read (iim, jjm, zlev_nm1, zlevuv_nm1, ttm, itau_read_nm1, itau_read_nm1, &
                       ...
                       &  SWnet_nm1, Eair_nm1, petAcoef_nm1, peqAcoef_nm1, petBcoef_nm1, peqBcoef_nm1, cdrag_nm1, ccanopy_nm1, &
                       &  force_id, wind_N_exists,read_ONfert, ONfert_nm1)
   ...
   !-----
            CALL forcing_just_read (iim, jjm, zlev_n, zlevuv_n, ttm, itau_read_n, itau_read_n, &
                 ...
                 &  SWnet_n, Eair_n, petAcoef_n, peqAcoef_n, petBcoef_n, peqBcoef_n, cdrag_n, ccanopy_n, &
                 &  force_id, wind_N_exists,read_ONfert, ONfert_n)
   ...
   !---
         ...
         ONfert(:,:) = (ONfert_n(:,:)-ONfert_nm1(:,:))*rw + ONfert_nm1(:,:)
...
!- qair       : 2m humidity (kg/kg)
!- pb         : Surface pressure (Pa)
!- ONfert     : ONfert data; stics fert data
!- lwdown     : Downward long wave radiation (W/m^2)
!-
!- From a WATCHOUT file :
!- SWnet      : Net surface short-wave flux
...
!- read_ONfert : flag to read ONfert data
!---------------------------------------------------------------------
   IMPLICIT NONE
!-
  ...
  &  swdown, rainf, snowf, tair, u, v, qair, pb, lwdown, ONfert
   ! for watchout files
   REAL, DIMENSION(iim,jjm), INTENT(out) :: &
  &  SWnet, Eair, petAcoef, peqAcoef, petBcoef, peqBcoef, cdrag, ccanopy
   INTEGER, INTENT(in) :: force_id
!  if Wind_N and Wind_E are in the file (and not just Wind)
   LOGICAL, INTENT(in) :: wind_N_exists, read_ONfert
   INTEGER :: i, j 
   REAL :: rau 
...
!---------------------------------------------------------------------
   ...
   IF (read_ONfert) THEN
     CALL flinget_buffer (force_id,'ONfert' , iim_full, jjm_full, llm_full, ttm, itb, ite, data_full) 
   ENDIF
   CALL forcing_zoom(data_full, ONfert)
