...
contains
    ...
    subroutine driver_stics(n,                               &  ! IN
                            ...
                            codesla, &
                            upvtutil, deltaimaxi,exolai, &
                            n_aground,c_aground,&
                            innlai_m,innsenes_m,inns_m,inn_m,nc_m,cnp_m)! added by moj


       implicit none

       ! DECLARATION
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!added by moj for the STICS nitrogen scheme      
    REAL(r_std), INTENT(IN)                    ::    n_aground

    REAL(r_std), INTENT(IN)                    ::    c_aground

    real,    intent(INOUT)   :: innlai_m   !> // OUTPUT // Index of nitrogenstress active on leaf growth // P_innmin à 1
    real,    intent(INOUT)   :: innsenes_m
    real,    intent(INOUT)   :: inns_m
    real,    intent(INOUT)   :: inn_m
    real,    intent(INOUT)   :: nc_m
    real,    intent(INOUT)   :: cnp_m
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       integer(i_std), intent(IN)                           :: nlevobs    ! the following variables ended with obs are only used for forcing simulation.  
       integer(i_std), intent(IN)                           :: namfobs    ! the initial value should be always 999
       integer(i_std), intent(IN)                           :: nfloobs  
       integer(i_std), intent(IN)                           :: nlanobs  
       integer(i_std), intent(IN)                           :: nlaxobs  
       integer(i_std), intent(IN)                           :: nmatobs  
       integer(i_std), intent(IN)                           :: nrecobs  
       integer(i_std), intent(IN)                           :: nsenobs  
       integer(i_std), intent(IN)                           :: ndrpobs  
      ...
      ! real(r_std),    intent(INOUT)                           :: upvtutil    !volumetric soil water content
...
!boxulai, boxndays, boxlai, boxlairem,boxtdev, boxbiom, boxbiomrem, boxdurage, boxsomsenbase
       ...
       real, intent(INOUT)                :: deltaimaxi 
       real, intent(INOUT)                :: exolai
...
!           histgrowth(:,:) = 0.
!       endif
!        WRITE(numout,*) '--------------before------stics.f90-------------' 
!        WRITE(numout,*) 'moj2: slai=',slai
!        WRITE(numout,*) '---------------------------------'
       call laidev(n,                               &  ! IN
                   in_cycle,                        &  ! inout
                   f_crop_recycle,                  &  ! INOUT
